 ...
 *
 * The function tries to copy the file in argument in a temporary folder, on a
 * random name. The file name is relative to the folder which descriptor is in
 * the global variable persistent.mirror_fd. If it succeeds, it returns a
 * newly-allocated string with the name of the temporary file. Otherwise, it
 * returns 0. \param file Source file which should be copied \return New string
 * with the name of the temporary file. The user is responsible for releasing
...
char *temp_copy(const char *file) {
    ...
    res = tempnam("/tmp", "sfs.");
    int fout =
        open(res, O_CREAT | O_WRONLY | O_TRUNC | O_EXCL, S_IRUSR | S_IXUSR);
    if (fout == -1) {
        close(fin);
        return 0;
    }
    ssize_t num;
    char buf[0x1000];
    while ((num = read(fin, &buf, 0x1000 * sizeof(char))) > 0)
        write(fout, &buf, num);
    close(fin);
    close(fout);
    return res;
...
int test_program(PTest test, const char *file) {
    // Create the array of arguments of the program by replacing the exclamation
    // mark with the name of the file
    if (test->args != 0 && test->filearg != 0) *(test->filearg) = (char *)file;
    // If the program is a filter that requires standard input, add the name of
    // the file in the arguments of the call to execute_program
...
int program_external(PProgram program, const char *file, int fd) {
    // Create the array of arguments of the program by replacing the exclamation
    // mark with the name of a file with the same content The actual file is not
    // used because it may not be accessible for external programs since the
    // host folder can be mounted over with the new file system. To prevent that
    // case, the script file is copied in the temporary folder and this new file
    // name is given as the argument of the external program at the location of
    // the exclamation mark. The temporary file is deleted after the end of the
    // procedure.
    if (program->args != 0 && program->filearg != 0)
        *(program->filearg) = temp_copy(file);
    // If the program is a filter that requires standard input, add the name of
    // the file in the arguments of the call to execute_program
    const char *f = (program->filter && program->filearg == 0) ? file : 0;
    // Launch the program
    int code =
        execute_program(program->path, (const char **)(program->args), fd, f);
...
Procedure *get_script(const Procedures *procs, const char *file) {
    Procedure *res = 0;
    while (res == 0 && procs != 0) {
        if (procs->procedure->test != 0 && procs->procedure->test->func != 0 &&
            procs->procedure->test->func(procs->procedure->test, file) != 0)
            res = procs->procedure;
        procs = procs->next;
    }
    return res;
...
int execute_program(const char *file, const char **args, int out,
    ...
    int fds[2];   // Handles of the two ends of the pipe, only used if input has
                 // to be provided to the standard input of the external program
    int in;
    if (path_in != 0)
        pipe(fds);  // Prepare a pipe to feed standard input of the external
                    // program, fork and copy the file to the pipe
    child = fork();
    if (child != 0) {        // Parent process (caller)
        if (path_in != 0) {  // If a path is provided, feed the content of the
                             // file to the pipe so that it is used as the
                             // standard input of the child process
            close(fds[0]);   // Close input descriptor
            in = openat(persistent.mirror_fd, path_in, O_RDONLY);
            if (in < 0)
                path_in = 0;
            else {  // Copy file to standard input
                char buffer[0x1000];
        ...
        if (out != 0)
            dup2(out, STDOUT_FILENO);  // Redirect output to out descriptor
        else
            dup2(STDERR_FILENO,
                 STDOUT_FILENO);  // Redirect standard output on standard error,
                                  // to avoid mixing outputs from the external
                                  // program and the parent process
        if (path_in == 0) {
            close(STDIN_FILENO);  // We do not want the external program to use
                                  // anything from the common standard input
        } else {
            close(fds[1]);  // Close output descriptor
            dup2(fds[0],
                 STDIN_FILENO);  // Redirect standard input to pipe output
        }
